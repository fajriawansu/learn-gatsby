...
export function finalDuality(
  ...
  const has33 = Object.values(persona).filter((v) => v >= 3).length >= 3;
  const highestPoint = Object.values(persona).reduce((a, b) => (a > b ? a : b));
  const mostDominatedPersona: ObservedEnum = Object.keys(persona)[
    highestPoint
  ] as unknown as ObservedEnum;

  console.log({ persona, last, has33, highestPoint, stringQnA });

  if (has33) {
    return CharacterReportEnum.all_rounder;
  } else {
